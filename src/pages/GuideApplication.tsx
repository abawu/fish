...
export default function GuideApplication() {
  ...
  const [selectedFiles, setSelectedFiles] = useState<{
    ...
    tourGuideCertificate: File | null;
  }>({
    nationalIdFront: null,
    nationalIdBack: null,
    personalPhoto: null,
    tourGuideCertificate: null,
  });
  ...
  const [filePreviews, setFilePreviews] = useState<{
    ...
    tourGuideCertificate: string | null;
  }>({
    nationalIdFront: null,
    nationalIdBack: null,
    personalPhoto: null,
  ...
  useEffect(() => {
    ...
    if (user?.guideStatus === "approved") {
      navigate("/profile");
      toast({
        title: "Already a guide",
        description: "You are already an approved guide.",
      });
      return;
    }
  ...
  const fetchApplication = async () => {
    try {
      setIsLoading(true);
      const response = await guideApplicationAPI.getMyApplication();
  ...
  const handleNext = async () => {
    if (currentStep === 1) {
      ...
      try {
        setIsLoading(true);
        await guideApplicationAPI.createOrUpdate(personalInfo);
        setCurrentStep(2);
      } catch (error: any) {
        toast({
          title: "Error",
      ...
      try {
        setIsLoading(true);
        await guideApplicationAPI.updateExperienceDetails(experienceDetails);
        setCurrentStep(3);
      } catch (error: any) {
        toast({
          title: "Error",
  ...
  const handleUploadFiles = async () => {
    ...
    if (selectedFiles.tourGuideCertificate) {
      formData.append('tourGuideCertificate', selectedFiles.tourGuideCertificate);
    }
    ...
    try {
      ...
      const response = await guideApplicationAPI.uploadMedia(formData);
      ...
      setMedia({
        ...
        tourGuideCertificate: uploadedFiles.tourGuideCertificate || media.tourGuideCertificate,
      });
  ...
  const handleSubmit = async () => {
    try {
      ...
      await guideApplicationAPI.submitApplication();
      toast({
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-800">
                    Your guide application has been submitted and is currently being reviewed by our team. 
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <Button
                    onClick={async () => {
                      try {
                        await guideApplicationAPI.reapplyApplication();
                        toast({
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Textarea
                          ...
                          placeholder="Tell us about yourself, your background, and why you want to become a guide..."
                          rows={5}
                          required
                        />
                      </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Textarea
                          ...
                          placeholder="Tell us about your previous experience as a tour guide or in the tourism industry..."
                          rows={5}
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <p className="text-sm text-blue-900">
                          Upload required documents: front and back of your National ID, a personal photo, and your government-issued tour guide certificate (max 5MB per file).
                        </p>
                      </div>
                      ...
                      <div>
                        <Label htmlFor="tourGuideCertificate">Tour Guide Certificate *</Label>
                        <Input
                          id="tourGuideCertificate"
                          type="file"
                          accept="image/*,application/pdf"
                          onChange={(e) => handleFileChange(e, 'tourGuideCertificate')}
                          className="cursor-pointer"
                        />
                        {filePreviews.tourGuideCertificate && (
                          <div className="mt-2">
                            {filePreviews.tourGuideCertificate.startsWith('data:application/pdf') ? (
                              <div className="p-4 border rounded-lg bg-gray-50">
                                <p className="text-sm text-gray-600">PDF file selected</p>
                              </div>
                            ) : (
                              <img 
                                src={filePreviews.tourGuideCertificate} 
                                alt="Tour guide certificate preview" 
                                className="w-full h-48 object-cover rounded-lg border"
                              />
                            )}
                          </div>
                        )}
                        {media.tourGuideCertificate && !filePreviews.tourGuideCertificate && (
                          <div className="mt-2 text-sm text-green-600">
                            ✓ Tour guide certificate uploaded
                          </div>
                        )}
                      </div>

                      {(selectedFiles.nationalIdFront || selectedFiles.nationalIdBack || selectedFiles.personalPhoto || 
                        selectedFiles.tourGuideCertificate) && (
                        <div className="pt-4">
                          <Button
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="text-sm space-y-1 text-muted-foreground">
                          ...
                          {media.tourGuideCertificate ? (
                            <p className="text-green-600">✓ Tour guide certificate uploaded</p>
                          ) : (
                            <p className="text-red-600">✗ Tour guide certificate required</p>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex justify-between mt-6">
                    ...
                    {currentStep < 4 ? (
                      ...
                      <Button
                        ...
                        disabled={isSubmitting || !media.nationalIdFront || !media.nationalIdBack || !media.personalPhoto || !media.tourGuideCertificate}
                        className="w-full sm:w-auto"
                      >
                        {isSubmitting ? (
                          <Loader2 className="w-4 h-4 mr-2 animate-spin" />
